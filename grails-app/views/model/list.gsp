			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="model" title="${message(code: 'model.model.label', default: 'Model')}" />
					
						<th><g:message code="model.make.label" default="Make" /></th>
				...
				<g:each in="${modelInstanceList}" status="i" var="modelInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${modelInstance.id}">${fieldValue(bean: modelInstance, field: "model")}</g:link></td>
					
						<td>${fieldValue(bean: modelInstance, field: "make")}</td>
